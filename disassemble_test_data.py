...
def try_upx_unpack(filepath):
    ...
    # Check for UPX in PATH, or locally in SCRIPT_DIR
    local_upx = os.path.join(SCRIPT_DIR, "upx.exe")
    upx_cmd = shutil.which("upx") or (os.path.exists(local_upx) and local_upx)
    if not upx_cmd:
        print(f"  [WARN] UPX not found. Cannot unpack: {os.path.basename(filepath)}")
        return False
    try:
        ...
        if result.returncode == 0:
            ...
            error_msg = result.stderr.strip().splitlines()[-1] if result.stderr else 'unknown error'
            if "NotPackedException" in error_msg:
                print(f"  [INFO] UPX signature found, but upx rejected it (likely spoofed by malware). Skipping unpack.")
            else:
                print(f"  [WARN] UPX unpack failed: {error_msg}")
            return False
...
PROJECT_DIR = os.path.join(SCRIPT_DIR, "ghidra", "projects", "MalwareProject")
    ...
    for root, dirs, files in os.walk(raw_folder):
        for binary in files:
            ...
            if os.path.exists(filtered_path):
                print(f"  [SKIP] Filtered out previously (0 insts): {binary[:16]}...")
                skipped += 1
                continue
